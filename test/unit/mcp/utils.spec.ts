...
describe("Server Utils", () => {
  describe("determineMcpParameterType", () => {
    ...
    describe("Deep Insert type handling", () => {
      // Store original cds reference
      const originalCds = (global as any).cds;
      ...
      test("should return empty array schema when model definitions are missing", () => {
        // Mock cds.model without definitions
        mockModel.definitions = null;

        // Import the function after mocking
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType("TestEntity");

        expect(z.array).toHaveBeenCalled();
        expect(z.object).toHaveBeenCalledWith({});
      });

      test("should return empty array schema when target entity name is undefined", () => {
        mockModel.definitions = {
          TestEntity: {
            elements: { field: { type: "String" } },
          },
        };

        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType(undefined);

        expect(z.array).toHaveBeenCalled();
        expect(z.object).toHaveBeenCalledWith({});
      });

      test("should return empty array schema when target entity doesn't exist", () => {
        mockModel.definitions = {
          SomeOtherEntity: {
            elements: { field: { type: "String" } },
          },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType("NonExistentEntity");

        expect(z.array).toHaveBeenCalled();
        expect(z.object).toHaveBeenCalledWith({});
      });

      test("should return empty array schema when target entity has no elements", () => {
        mockModel.definitions = {
          EmptyEntity: {
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType("EmptyEntity");

        expect(z.array).toHaveBeenCalled();
        expect(z.object).toHaveBeenCalledWith({});
      });

      test("should build schema for simple entity with basic types", () => {
        mockModel.definitions = {
          BookingItem: {
            elements: {
              id: { type: "cds.UUID", key: true, notNull: true },
              description: { type: "cds.String", key: false, notNull: true },
              quantity: { type: "cds.Integer", key: false, notNull: false },
            },
          },
        };

        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType("BookingItem");

        expect(z.object).toHaveBeenCalled();
        expect(z.array).toHaveBeenCalled();
      });

      test("should skip computed fields in deep insert schema", () => {
        mockModel.definitions = {
          Product: {
            elements: {
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("Product");

        // Should only process non-computed fields
        expect(z.object).toHaveBeenCalled();
      });

      test("should skip association fields in deep insert schema", () => {
        mockModel.definitions = {
          Order: {
            elements: {
              id: { type: "cds.UUID", key: true },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("Order");

        expect(z.object).toHaveBeenCalled();
      });

      test("should skip composition fields in deep insert schema", () => {
        mockModel.definitions = {
          Order: {
            elements: {
              id: { type: "cds.UUID", key: true },
      ...
      test("should handle elements without type property", () => {
        mockModel.definitions = {
          TestEntity: {
            elements: {
              validField: { type: "cds.String" },
              invalidField: {
                // Missing type property
                key: false,
              },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("TestEntity");

        // Should only process fields with type
        expect(z.object).toHaveBeenCalled();
      });
      ...
      test("should mark optional fields correctly", () => {
        mockModel.definitions = {
          Customer: {
            elements: {
              id: { type: "cds.UUID", key: true, notNull: true },
              name: { type: "cds.String", key: false, notNull: true },
              email: { type: "cds.String", key: false, notNull: false },
              phone: { type: "cds.String" }, // No key, no notNull = optional
            },
          },
        };

        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("Customer");

        // Should call optional() for non-required fields
        expect(mockOptional).toHaveBeenCalled();
        expect(z.object).toHaveBeenCalled();
        expect(z.array).toHaveBeenCalled();
      });

      test("should handle all CDS data types", () => {
        mockModel.definitions = {
          ComplexEntity: {
            elements: {
              uuid: { type: "cds.UUID" },
              str: { type: "cds.String" },
              int: { type: "cds.Integer" },
              decimal: { type: "cds.Decimal" },
              bool: { type: "cds.Boolean" },
              date: { type: "cds.Date" },
              time: { type: "cds.Time" },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("ComplexEntity");

        expect(z.object).toHaveBeenCalled();
        expect(z.array).toHaveBeenCalled();
      });

      test("should always return array type for deep insert", () => {
        mockModel.definitions = {
          Item: {
            elements: {
              id: { type: "cds.UUID", key: true },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        const result = buildDeepInsertZodType("Item");

        // Should always wrap in array
        expect(z.array).toHaveBeenCalled();
      });

      test("should handle entity with mixed field requirements", () => {
        mockModel.definitions = {
          Product: {
            elements: {
              id: { type: "cds.UUID", key: true, notNull: true },
              sku: { type: "cds.String", key: true, notNull: true },
              name: { type: "cds.String", notNull: true },
              description: { type: "cds.String", notNull: false },
      ...
      test("should handle case-insensitive @Core.Computed annotation", () => {
        mockModel.definitions = {
          Entity: {
            elements: {
              field1: {
                type: "cds.String",
                "@core.computed": true, // lowercase
              },
              field2: {
                type: "cds.String",
                "@Core.Computed": true, // proper case
              },
              field3: {
                type: "cds.String",
                "@CORE.COMPUTED": true, // uppercase
              },
              validField: { type: "cds.String" },
            },
          },
        ...
        const { buildDeepInsertZodType } = require("../../../src/mcp/utils");
        buildDeepInsertZodType("Entity");

        // Should skip all computed fields regardless of case
        expect(z.object).toHaveBeenCalled();
      });
    });
  });
